        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
          ...
          <div className="lg:col-span-4 lg:text-right">
            <p className="text-gray-500 text-xs md:text-sm leading-relaxed max-w-sm ml-auto">
              A systematic collection of digital artifacts, ranging from high-fidelity interfaces to complex engineering systems. Built with precision and intent.
            </p>
          </div>
        </div>
        ...
        <div className="mt-10 sm:mt-16 flex flex-col sm:flex-row flex-wrap items-start sm:items-center justify-between border-y border-white/5 py-4 sm:py-6 gap-4 sm:gap-6">
          <div className="flex flex-wrap gap-4 sm:gap-8 text-[10px] font-bold tracking-[0.2em] uppercase">
            {(['all', 'engineering', 'design'] as Filter[]).map((f) => (
              ...
              >
                {f === 'all' ? 'All_Systems' : f === 'engineering' ? 'Engineering' : 'UI/UX_Design'}
              </button>
            ))}
          </div>
          <div className="text-[10px] text-gray-600 font-mono">
            TOTAL_ENTRIES: <span className="text-white">{String(visible.length).padStart(2, '0')}</span>
          </div>
        </div>
      </header>
              ...
              <div className="p-5 sm:p-8">
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tech.slice(0, 3).map((tag) => (
                    <span key={tag} className="text-[9px] text-gray-500 font-mono border border-white/5 px-2 py-0.5">{tag}</span>
                  ))}
                </div>
                <h3 className="text-xl font-bold mb-3 tracking-tight group-hover:text-accent transition-colors uppercase">
                  {project.title}
                </h3>
                <p className="text-gray-500 text-xs leading-relaxed mb-6">{project.description}</p>
                ...
                <div className="flex items-center justify-between pt-6 border-t border-white/5">
                  ...
                  <span className="text-[10px] text-gray-700 font-mono">0{idx + 1}_</span>
                </div>
              </div>
            </motion.article>
          ))}
      ...
      <section className="max-w-[1400px] mx-auto w-full mt-20 sm:mt-40">
        <div className="bento-card p-6 sm:p-12 bg-white/[0.01] border-dashed border-white/10 text-center">
          <div className="max-w-xl mx-auto">
            <span className="text-accent text-[10px] font-bold uppercase tracking-[0.5em] mb-4 block">Experimental_Zone</span>
            <h2 className="text-2xl sm:text-3xl font-bold mb-6 italic">&ldquo;Design is a function of logic, refined by empathy.&rdquo;</h2>
      ...
      <footer className="py-10 sm:py-12 px-4 sm:px-6 border-t border-white/5 bg-black/40 backdrop-blur-md relative z-10 mt-16 sm:mt-20">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex flex-col gap-2">
            <span className="text-xs font-bold tracking-widest text-accent">JETHRO ADEBISI</span>
            <span className="text-[10px] text-gray-600 font-mono uppercase tracking-tighter">Handcrafted in Lagos / Built for the Web</span>
          </div>
          <div className="flex flex-wrap justify-center gap-6 sm:gap-10 text-[10px] font-bold tracking-widest uppercase">
            <a href="https://x.com/jethroadebisi" target="_blank" rel="noopener noreferrer" className="hover:text-accent transition-colors">Twitter_X</a>
            <a href="#" className="hover:text-accent transition-colors">LinkedIn</a>
            <a href="#" className="hover:text-accent transition-colors">GitHub</a>
          </div>
          <Link href="/" className="text-[10px] text-gray-700 font-mono hover:text-accent transition-colors">
            ← Back to Gateway
          </Link>
        </div>
      </footer>
...
function RotateBox() {
  ...
  return (
    ...
    >
      <span className={`text-xl transition-colors ${active ? 'text-accent' : 'text-gray-700'}`}>∫</span>
    </div>
  );
}
...
function CounterBox() {
  ...
  return (
    ...
    >
      <span className="text-[8px] group-hover:text-black transition-colors">INC_</span>
      <span className="text-xs font-bold group-hover:text-black transition-colors">{count}</span>
    </div>
  );
}
